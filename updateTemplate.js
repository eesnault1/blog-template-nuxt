...
async function main() {
    ...
    const EXCLUDE = new Set(["public/", ".env", "articles/", "styles/main.css", "updateTemplate.js", ".git"]);
...
const copyFilesRecursively = async (startPath) => {
    ...
    for (const fileOrDir of filesAndDirs) {
        ...
        if (stats.isFile()) {
            // ... vérifier s'il n'est pas dans la liste des éléments à exclure.
            if (!EXCLUDE.has(filePath.replace(TMP_DIR + "/", ""))) {
